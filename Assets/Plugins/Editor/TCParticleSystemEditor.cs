...
public class TCParticleSystemEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		if (o.ToggleArea("Particle System"))
		{
			...
			if (fmanager.BaseForces != null)
			{
				...
				for (int i = 0; i < fmanager.BaseForces.Count; ++i)
				{
					...
					fmanager.BaseForces[i] =
						EditorGUILayout.ObjectField("Link to ", (Object) fmanager.BaseForces[i], typeof (TCForce), true) as TCForce;
		...
		if (o.ToggleArea("Collision"))
		{
			...
			for (int i = 0; i < baseColliders.Count; ++i)
			{
				...
				GUILayout.Space(20.0f);
				baseColliders[i] =
					EditorGUILayout.ObjectField("Link to ", (Object) baseColliders[i], typeof (TCCollider), true) as TCCollider;
	...
	public void OnSceneGUI()
	{
		...
		switch (e.Shape)
		{
			...
			case EmitShapes.Mesh:
				...
				GL.PushMatrix();
				GL.MultMatrix(syst.transform.localToWorldMatrix);
				GL.Begin(GL.LINES);
